    ...
    Config {
        rg: RgConfig {
            paths: proto_config
                .rg
                .as_ref()
                .map_or_else(Vec::new, |rg| rg.paths.clone()),
            ignore: proto_config.rg.as_ref().map_or_else(
                || None,
                |rg| {
                    if rg.ignore.is_empty() {
                        None
                    } else {
                        Some(rg.ignore.clone())
                    }
                },
            ),
            file_types: proto_config.rg.as_ref().map_or_else(
                || None,
                |rg| {
                    if rg.file_types.is_empty() {
                        None
                    } else {
                        Some(rg.file_types.clone())
                    }
                },
            ),
        },
        projects: proto_config
            .projects
            .into_iter()
            .map(|(k, v)| {
                (
        ...
        todo_done_pairs: proto_config
            .todo_done_pairs
            .into_iter()
            .map(|p| vec![p.todo_marker, p.done_marker])
            .collect(),
...
fn to_proto_config(config: &Config) -> ProtoConfigMessage {
    ProtoConfigMessage {
        ...
        todo_done_pairs: config
            .todo_done_pairs
            .iter()
            .filter_map(|p| {
                if p.len() == 2 {
                    Some(ProtoTodoDonePair {
                        todo_marker: p[0].clone(),
                        done_marker: p[1].clone(),
                    })
                } else {
                    None
                }
...
fn generate_short_timestamp() -> String {
    ...
    base64_timestamp.push(
        url_safe_base64_chars
            .chars()
            .nth(((timestamp_value >> 24) & mask6bit) as usize)
            .unwrap_or('A'),
    );
    base64_timestamp.push(
        url_safe_base64_chars
            .chars()
            .nth(((timestamp_value >> 18) & mask6bit) as usize)
            .unwrap_or('A'),
    );
    base64_timestamp.push(
        url_safe_base64_chars
            .chars()
            .nth(((timestamp_value >> 12) & mask6bit) as usize)
            .unwrap_or('A'),
    );
    base64_timestamp.push(
        url_safe_base64_chars
            .chars()
            .nth(((timestamp_value >> 6) & mask6bit) as usize)
            .unwrap_or('A'),
    );
    base64_timestamp.push(
        url_safe_base64_chars
            .chars()
            .nth((timestamp_value & mask6bit) as usize)
            .unwrap_or('A'),
    );
    base64_timestamp
}
...
fn extract_cleaned_content_from_line(
    line: &str,
    ...
    let todo_pattern_re = Regex::new(effective_rg_pattern).map_err(|e| {
        io::Error::new(
            ...
            format!(
                "Invalid effective regex pattern in config for extraction: {}",
                e
            ),
        )
    })?;
...
impl Sink for TodoSink {
    ...
    fn matched(&mut self, _searcher: &Searcher, mat: &SinkMatch<'_>) -> Result<bool, io::Error> {
        ...
        let todo_pattern_re = match Regex::new(&self.effective_rg_pattern) {
            ...
            Err(_) => {
                eprintln!(
                    "Error: Invalid regex pattern in sink (\'{}\')",
                    self.effective_rg_pattern
                );
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "Invalid regex pattern in sink",
                ));
            }
        };
            ...
            if !project_match {
                match find_git_repo_name(file_path) {
                    ...
                    Err(e) => {
                        if self.debug {
                            eprintln!(
                                "[Sink] Warning: Failed to check git repo for {}: {}",
                                file_path.display(),
                                e
                            );
                        }
...
fn find_and_process_todos(config: &Config, debug: bool) -> io::Result<ProcessedTodosOutput> {
    ...
    if debug {
        println!(
            "[{:.2?}] Using effective search pattern: {}",
            start_time.elapsed(),
            effective_pattern
        );
    }
    ...
    let matcher = match RegexMatcher::new(&effective_pattern) {
        ...
        Err(e) => {
            eprintln!(
                "Error: Invalid effective regex pattern derived from config (\'{}\'): {}",
                effective_pattern, e
            );
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("Invalid effective regex pattern: {}", e),
            ));
        }
    };
    ...
    if let Some(items_to_ignore) = &config.rg.ignore {
        for pattern_str in items_to_ignore {
            match Glob::new(pattern_str) {
                Ok(glob) => {
                    custom_ignore_builder.add(glob);
                }
                Err(e) => {
                    if debug {
                        eprintln!(
                            "[{:.2?}] Warning: Invalid custom ignore glob pattern '{}': {}",
                            start_time.elapsed(),
                            pattern_str,
                            e
                        );
    ...
    let custom_ignores = match custom_ignore_builder.build() {
        Ok(gs) => {
            if debug && !gs.is_empty() {
                println!(
                    "[{:.2?}] Using custom ignore patterns from config.",
                    start_time.elapsed()
                );
            }
            gs
        ...
        Err(e) => {
            if debug {
                eprintln!(
                    "[{:.2?}] Warning: Failed to build custom ignore GlobSet: {}",
                    start_time.elapsed(),
                    e
                );
            }
    ...
    if let Some(types) = &config.rg.file_types {
        if debug {
            eprintln!(
                "Warning: config `rg.file_types` ('{:?}') not yet supported with internal search.",
                types
            );
        }
    }
    ...
    let grouped_todos = Arc::new(ParkingMutex::new(
        HashMap::<TodoCategoryEnum, Vec<TodoItem>>::new(),
    ));
        ...
        Box::new(move |result| {
            ...
            if entry.file_type().map_or(false, |ft| ft.is_file()) {
                ...
                if let Err(err) = result {
                    if is_debug {
                        eprintln!(
                            "[{:.2?}] Warning: Error searching file {}: {}",
                            start_time_ref.elapsed(),
                            path.display(),
                            err
                        );
                    }
    ...
    let final_grouped_todos = match Arc::try_unwrap(grouped_todos) {
        ...
        Err(_) => {
            eprintln!(
                "Error: Could not obtain exclusive access to grouped_todos after parallel walk."
            );
            return Err(io::Error::new(
                io::ErrorKind::Other,
    ...
    if debug {
        println!(
            "[{:.2?}] Output processed and structured in {:.2?}",
            start_time.elapsed(),
            format_output_start.elapsed()
        );
        println!(
            "[{:.2?}] Total processing time: {:.2?}",
            start_time.elapsed(),
            start_time.elapsed()
        );
    }
...
fn edit_todo_in_file_grpc(
    ...
    let line_number: usize = location_parts[1]
        .parse()
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Invalid line number"))?;
    if line_number == 0 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
    ...
    let result = (|| {
        ...
        let todo_pattern_re = Regex::new(&effective_rg_pattern)
            .map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "bad regex"))?;

        if let Some(mat) = todo_pattern_re.find(original_line) {
            let prefix = &original_line[..mat.start()];
            let pattern_match_str = mat.as_str();
            let content_start_idx = original_line[mat.end()..]
                .find(|c: char| !c.is_whitespace())
                .map_or(original_line.len(), |i| mat.end() + i);
            ...
            let final_write_content =
                if file_content_string.ends_with('\n') && !new_full_content.is_empty() {
                    format!("{}\n", new_full_content)
                } else {
                    new_full_content
                };
...
fn add_todo_to_file_grpc(
    config: &Config,
    category_type: &str,
    category_name: &str,
    content: &str,
    example_item_location: Option<&str>,
) -> io::Result<()> {
    let target_append_file_path: PathBuf = match category_type {
        "git" => {
            let ex_loc = example_item_location.ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "Missing example_item_location for git",
                )
            })?;
            let ex_path_str = ex_loc.split(':').next().ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "Invalid example_item_location format",
                )
            })?;
            let repo_root = find_git_repo_root(Path::new(ex_path_str))?.ok_or_else(|| {
                io::Error::new(io::ErrorKind::NotFound, "Could not find git repo root")
            })?;
            get_append_file_path_in_dir(&repo_root, &config.default_append_basename)
        }
        "project" => {
            let proj_conf = config.projects.get(category_name).ok_or_else(|| {
                io::Error::new(io::ErrorKind::NotFound, "Project config not found")
            })?;
            PathBuf::from(proj_conf.append_file_path.as_ref().ok_or_else(|| {
                io::Error::new(io::ErrorKind::NotFound, "append_file_path not configured")
            })?)
        }
        _ => {
            return Err(io::Error::new(
    ...
    let base_line_to_append = format!(
        "{} {}@{} {}",
        priority_marker, effective_priority_segment, timestamp_str, content_segment
    )
    .trim_end()
    .to_string();
...
fn mark_todo_as_done_in_file_grpc(
    config: &Config,
    location: &str,
    original_content_payload: &str,
    ...
    let line_number: usize = location_parts[1]
        .parse()
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Invalid line num"))?;
    if line_number == 0 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "Line number cannot be 0",
    ...
    let result: Result<(String, bool), io::Error> = (|| {
        ...
        let mut lines: Vec<String> = original_file_content_string
            .lines()
            .map(String::from)
            .collect();
        if line_index >= lines.len() {
        ...
        let effective_rg_pattern = config.get_effective_rg_pattern();
        let current_disk_cleaned_content =
            extract_cleaned_content_from_line(&original_line_on_disk, &effective_rg_pattern)?;
        if current_disk_cleaned_content.trim() != original_content_payload.trim() {
            return Err(io::Error::new(
                io::ErrorKind::Other,
                "Content modified since load",
            ));
        }

        let todo_done_pairs = if config.todo_done_pairs.is_empty() {
            default_todo_done_pairs()
        } else {
            config.todo_done_pairs.clone()
        };
        let mut marker_transformed = false;

        let marker_re = Regex::new(&effective_rg_pattern)
            .map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "marker regex error"))?;
        ...
        if let Some(mat) = marker_re.find(&original_line_on_disk) {
            ...
            let leading_space_after_marker_len =
                content_after_marker_with_space.len() - actual_content_to_timestamp.len();
            let leading_space_after_marker =
                &content_after_marker_with_space[0..leading_space_after_marker_len];

            let parts: Vec<&str> = actual_content_to_timestamp.splitn(2, ' ').collect();
            let mut first_word_of_content =
                parts.get(0).map_or(String::new(), |s| (*s).to_string());
            ...
            if done_ts_regex.is_match(&first_word_of_content) {
                first_word_of_content = done_ts_regex
                    .replace(&first_word_of_content, &done_ts_str)
                    .to_string();
            } else {
            ...
            final_line_to_write = format!(
                "{}{}{}{}",
                prefix_before_marker,
                transformed_marker_str,
                leading_space_after_marker,
                final_content_part_with_timestamp
            );
        ...
        let new_full_content = lines.join("\n");
        let final_write_content =
            if original_file_content_string.ends_with('\n') && !new_full_content.is_empty() {
                format!("{}\n", new_full_content)
            } else {
                new_full_content
            };
...
impl TodoService for MyTodoService {
        ...
        match find_and_process_todos(&config_guard, false) {
            Ok(processed_data) => {
                let proto_categories = processed_data
                    ...
                    .collect();
                Ok(Response::new(GetTodosResponse {
    ...
    async fn edit_todo(
        ...
        match edit_todo_in_file_grpc(
            ...
            Ok(()) => Ok(Response::new(EditTodoResponse {
                status: "success".to_string(),
                message: "Todo edited successfully".to_string(),
            })),
            Err(e) => {
                let (code, msg) = match e.kind() {
                    ...
                    io::ErrorKind::PermissionDenied => {
                        (tonic::Code::PermissionDenied, e.to_string())
                    }
                    io::ErrorKind::Other if e.to_string().contains("Content has been modified") => {
                        (tonic::Code::Aborted, e.to_string())
                    }
    ...
    async fn add_todo(
        ...
        match add_todo_to_file_grpc(
            ...
            &payload.content,
            payload.example_item_location.as_deref(),
        ) {
            Ok(()) => Ok(Response::new(AddTodoResponse {
                status: "success".to_string(),
        ...
        match mark_todo_as_done_in_file_grpc(
            ...
            Err(e) => {
                let (code, msg) = match e.kind() {
                    ...
                    io::ErrorKind::Other
                        if e.to_string().contains("Content modified since load") =>
                    {
                        (tonic::Code::Aborted, e.to_string())
                    }
                    _ => (
                        tonic::Code::Internal,
                        format!("Failed to mark todo as done: {}", e),
                    ),
                };
                Err(Status::new(code, msg))
            }
...
impl ConfigService for MyConfigService {
    ...
    async fn update_config(
        ...
        let proto_config_to_save = request
            .into_inner()
            .config
...
// --- Tauri Commands ---
#[tauri::command]
async fn get_config_command(
    config_state: tauri::State<'_, Arc<RwLock<Config>>>,
) -> Result<ProtoConfigMessage, String> {
    ...
    match service.get_config(request).await {
        Ok(response) => response
            .into_inner()
            .config
            .ok_or_else(|| "Config data missing in response".to_string()),
        Err(status) => Err(status.to_string()),
    }
}

#[tauri::command]
async fn update_config_command(
    new_config_payload: ProtoConfigMessage,
    config_state: tauri::State<'_, Arc<RwLock<Config>>>,
